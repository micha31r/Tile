...
-- Trigger function: block insert if 4 already exist for the same user & UTC date
...
DECLARE
  ...
  v_lock_key bigint;
BEGIN
  -- Work in UTC date space (handles timestamptz correctly)
  v_day := COALESCE((NEW.created_at AT TIME ZONE 'UTC')::date,
  ...
  -- Count existing rows for same user on the same UTC date
  SELECT COUNT(*)
    INTO v_count
  FROM public.goal g
  WHERE g.user_id = NEW.user_id
    AND (g.created_at AT TIME ZONE 'UTC')::date = v_day;

  IF v_count >= 4 THEN
    RAISE EXCEPTION 'Daily limit of 4 reached for user % on % (UTC)', NEW.user_id, v_day
      USING ERRCODE = '23505';
  END IF;
